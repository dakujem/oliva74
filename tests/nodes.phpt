...
(function () {
    $builder = new NodeBuilder(fn($data) => new Node($data));
...
(function () {
    ...
    Tree::linkChildren($d, $e, null,  function (TreeNodeContract $originalParent) use (&$hasRun, $b) {
        $hasRun = true;
...
(function () {
    ...
    Assert::throws(function () {
        $builder = new NodeBuilder(fn( $data) => new NotMovable($data));
        $builder->node('root', [
            'foo',
        ]);
