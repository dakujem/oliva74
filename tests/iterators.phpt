...
(function () {
    ...
    $iterator = new PreOrderTraversal(
        $root,
        fn(TreeNodeContract $node, array $vector, int $seq, int $counter): int => $counter + 1,
    );
    $expected = [
        1 => 'F',
        'B',
    ...
    $iterator = new PreOrderTraversal(
        $root,
        fn(TreeNodeContract $node, array $vector, int $seq, int $counter): string => '.' . implode('.', $vector),
    );
    $expected = [
        '.' => 'F',
        '.0' => 'B',
